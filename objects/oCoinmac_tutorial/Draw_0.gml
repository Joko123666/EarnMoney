...
if (state == tutorialCoinMachineState.TOSSING) {
...
} else if (state == tutorialCoinMachineState.RESULT || state == tutorialCoinMachineState.SETTLEMENT) {
    ...
    if (state == tutorialCoinMachineState.RESULT) {
...
if (state == tutorialCoinMachineState.BETTING) {
    ...
    // 재도전 버튼 (튜토리얼에서는 항상 표시하지 않음, 혹은 조건부)
    // if (check_artifact("Coin_iron") && !reroll_used_this_turn) {
